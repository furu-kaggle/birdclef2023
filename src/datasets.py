...
class WaveformDataset(Dataset):
    def __init__(self,
        ...
        cand_df = self.df[["filename_id","latitude","longitude"]].drop_duplicates().dropna()
        cand_df = cand_df[cand_df.latitude != 'Not specified'].reset_index(drop=True)
        print(cand_df.info())
        cand_df["latitude"] = cand_df["latitude"].astype(float).round(2)
        cand_df["longitude"] = cand_df["longitude"].astype(float).round(2)
